...
type Voice = {
  ...
  dcSource: ConstantSourceNode;
  thresholder: WaveShaperNode;
  lastControlRegister: number;
};
...
function setVoicePulseWidth(voice: number) {
...
function setControlRegister(voice: number, byte: number) {
...
function setVoiceVolume(voice: number, volume: number) {
  voices[voice].voiceGain.gain.value = volume;
}

function onRegWrite(reg: number, byte: number) {
...
export type AudioHost = {
  ...
  userDidInteract: () => void,
  setUiGain: (value: number) => void,
};
...
function setUiGain(value: number) {
