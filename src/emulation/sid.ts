/*
   ...
   Adapted from https://github.com/luxocrates/viciious project, and converted
   to TypeScript.
...
import { type AudioHost } from "./audio-OscillatorNode";

function unimplementedWarning(_: string) {}
...
function getMultiplier(asymptote: number, steps: number) {
...
// Bound by `attach`
let regWriteToHost: (reg: number, value: number) => void = () => {};
let setVolumeToHost: (voice: number, value: number) => void = () => {};
...
function makeDefaultAdsr(voice: number): Adsr {
  return {
    voice,
    phase: "off",
    value: 0,
...
type State = {
  regs: Uint8Array<ArrayBuffer>,
  adsr: [Adsr, Adsr, Adsr],
  cyclesUntilAdsrTick: number;
};
...
let state: State = makeDefaultState();


function toBridge(adsr: Adsr) {
...
function stop(adsr: Adsr) {
  adsr.phase = "off";
  adsr.value = 0;
  toBridge(adsr);
}

function keyon(adsr: Adsr) {
  adsr.phase = "attack";
...
function read_d400_d7ff(addr: number) {
  const reg = addr & 0x1f;
...
function write_d400_d7ff(addr: number, byte: number) {
...
export type Sid = {
  tick: () => void;
  reset: () => void;
  read_d400_d7ff: (addr: number) => number;
  write_d400_d7ff: (addr: number, byte: number) => void;
};

export function createSid(audioHost: AudioHost): Sid {
  regWriteToHost  = audioHost.onRegWrite;
  setVolumeToHost = audioHost.setVoiceVolume;
